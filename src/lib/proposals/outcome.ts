...
export async function contractFromProposal(actor: Actor, proposalId: string, months = 12) {
  ...
  const c = await prisma.contract.create({ data: { contractNumber, name: `${p.account.name} — ${p.reference}`, type: "LOCAL", status: "ACTIVE", accountId: p.accountId, currency: p.currency, effectiveFrom: from, effectiveTo: to, sourceSystem: "crosswalk", externalId: p.id, ownerUserId: p.ownerUserId, createdByUserId: actor.id, notes: `Created from won proposal ${p.reference}` } });
  for (const l of p.lines) {
    if (!l.included || !l.productId || !money(l.proposedPrice)) continue;
    await prisma.priceEntry.create({ data: { contractId: c.id, accountId: p.accountId, productId: l.productId, productFamily: l.productFamily, price: toDb(l.proposedPrice)!, currency: p.currency, effectiveFrom: from, effectiveTo: to, source: "proposal", status: "ACTIVE", approvalState: "APPROVED" } });
    await prisma.contractCommitment.create({ data: { contractId: c.id, productId: l.productId, productFamily: l.productFamily, committedUnits: toDb(l.quantity), committedValue: toDb(money(l.quantity)!.times(money(l.proposedPrice)!)), periodStart: from, periodEnd: to } });
  }
