  ...
  for (const p of products) {
    ...
    try {
      ...
      if (!best) {
        ...
        const bin = heuristicBin({ sku: p.sku, brand: s.brand, description: `${p.description} ; ${s.description ?? ""}`, category: p.category, gmdnName: s.gmdnName, sizes: s.sizes, singleUse: s.singleUse, sterile: s.sterile, implantable: s.implantable });
