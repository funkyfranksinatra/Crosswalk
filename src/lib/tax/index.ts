...
import { money, round, times, toDb, ZERO, type Money } from "@/lib/money";
...
export async function quoteTotals(proposalId: string) {
  ...
  const latestLine = p.lines.reduce((m, l) => (l.updatedAt > m ? l.updatedAt : m), new Date(0));
  // A line priced after the last calculation makes the tax figure stale (shown as "recalculate", never silently reused).
  const stale = (p.taxMode === "MANUAL" || p.taxMode === "PROVIDER") && (!p.taxCalculatedAt || p.taxCalculatedAt < latestLine);
  return { currency: p.currency, subtotal, freight, freightMode: p.freightMode, tax, taxMode: p.taxMode, total, taxCalculatedAt: p.taxCalculatedAt, taxStale: stale, taxNote: p.taxDetailJson ? (JSON.parse(p.taxDetailJson) as { note?: string | null }).note ?? null : null };
}
...
export async function setProposalLogistics(actor: Actor, proposalId: string, input: LogisticsInput) {
  ...
  if (input.taxRate !== undefined) {
    ...
    data.taxRate = toDb(r);
    if (((data.taxMode as string) ?? p.taxMode) === "MANUAL") { data.taxAmount = null; data.taxCalculatedAt = null; }
  }
  if (input.taxExemptionNo !== undefined) data.taxExemptionNo = input.taxExemptionNo ? String(input.taxExemptionNo).slice(0, 80) : null;
  if (input.shipTo !== undefined) { const a = parseAddress(input.shipTo); data.shipToJson = a ? JSON.stringify(a) : null; if ((((data.taxMode as string) ?? p.taxMode) === "PROVIDER")) { data.taxAmount = null; data.taxCalculatedAt = null; } }
  // Freight is taxable in some jurisdictions: a freight change invalidates a calculated tax figure.
  if ((input.freightMode !== undefined || input.freightValue !== undefined) && ["MANUAL", "PROVIDER"].includes((data.taxMode as string) ?? p.taxMode)) { data.taxAmount = null; data.taxCalculatedAt = null; }
...
export async function calculateProposalTax(actor: Actor, proposalId: string): Promise<{ result: TaxResult; totals: Awaited<ReturnType<typeof quoteTotals>> }> {
  ...
  await prisma.proposal.update({ where: { id: proposalId }, data: { taxAmount: toDb(totalTax), freightAmount: toDb(freight), taxCalculatedAt: new Date(), taxProvider: result.provider, taxDetailJson: JSON.stringify({ note: result.note ?? null, totalTaxable: result.totalTaxable, totalExempt: result.totalExempt, lines: result.lines, summary: result.summary, raw: result.raw ?? null, shipTo, freight: freight.toString(), subtotal: subtotal.toString() }) } });
