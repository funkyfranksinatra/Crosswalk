...
export function CatalogTable({ products, categories, q, cat, only, total, showPrice = true, showCost = true }: { products: Product[]; categories: { name: string; count: number }[]; q: string; cat: string; only: string; total: number; showPrice?: boolean; showCost?: boolean }) {
  ...
  return (
    <>
      <div className="flex items-center gap-2 px-4 py-3 border-b border-line-2 flex-wrap">
        <select className="input w-auto" aria-label="Category" value={cat} onChange={(e) => nav({ cat: e.target.value })}>
          <option value="">All categories</option>
          {categories.map((c) => <option key={c.name} value={c.name}>{c.name} ({c.count})</option>)}
        </select>
        <select className="input w-auto" aria-label="Show only" value={only} onChange={(e) => nav({ only: e.target.value })}>
          <option value="">Everything</option>
          <option value="unpriced">Unpriced</option>
          <option value="nogudid">No GUDID record</option>
          <option value="discontinued">Discontinued</option>
        </select>
        <input className="input w-full sm:w-[280px]" aria-label="Search SKU, description, brand" placeholder="Search SKU, description, brand…" value={search} onChange={(e) => setSearch(e.target.value)} />
      ...
      {products.length === 0 ? <Empty title="No products match" /> : (
        <table className="table">
          <thead><tr><th>SKU</th><th>Description</th><th>Category</th><th>GUDID</th>{showPrice && <th className="text-right">List</th>}{showCost && <th className="text-right">COGS</th>}{showPrice && <th>Pricebooks</th>}<th>Bin</th></tr></thead>
          <tbody>
            {products.map((p) => {
              const bin = safeBin(p.binJson);
              const isOpen = open === p.id;
              return (
                <RowGroup key={p.id} p={p} bin={bin} isOpen={isOpen} onToggle={() => setOpen(isOpen ? null : p.id)} showPrice={showPrice} showCost={showCost} />
              );
            })}
          </tbody>
        </table>
...
function RowGroup({ p, bin, isOpen, onToggle, showPrice, showCost }: { p: Product; bin: Bin | null; isOpen: boolean; onToggle: () => void; showPrice: boolean; showCost: boolean }) {
  const cols = 5 + (showPrice ? 2 : 0) + (showCost ? 1 : 0);
  return (
    <>
      <tr className={`cursor-pointer ${isOpen ? "bg-panel-2" : ""}`} onClick={onToggle}>
        <td><button type="button" className="mono font-semibold text-left" aria-expanded={isOpen} onClick={(e) => { e.stopPropagation(); onToggle(); }}>{p.sku}</button>{p.brand && <div className="text-[11.5px] text-muted">{p.brand}</div>}</td>
        ...
        {showPrice && <td className="mono text-right">{money(p.listPrice)}</td>}
        {showCost && <td className="mono text-right">{money(p.cogs)}</td>}
        {showPrice && <td className="text-[12px]">{p.prices.length ? p.prices.map((e) => <div key={e.name}><span className="text-muted">{e.name}</span> <span className="mono">{money(e.price)}</span></div>) : <span className="text-faint">—</span>}</td>}
        <td>{bin ? <Chip tone={p.binSource === "llm" ? "accent" : "neutral"}>{p.binSource === "llm" ? "model" : "heuristic"}</Chip> : <span className="text-faint">—</span>}</td>
      </tr>
      {isOpen && (
        <tr className="bg-panel-2"><td colSpan={cols} className="!pt-0">
...
export function CatalogActions({ canManage = true, canImportCost = true }: { canManage?: boolean; canImportCost?: boolean }) {
  ...
  useDismiss(panel !== null, closePanel, popover);

  // Poll the enrich job every 1.5 s while the panel is open and the tab is visible. A reply
  // that arrives after a newer one (or after the panel closed) is dropped, so a slow earlier
  // poll can never overwrite fresher progress.
  useEffect(() => {
    if (panel !== "enrich") return;
    let seq = 0, latest = 0, stopped = false, wasRunning = false;
    const t = setInterval(async () => {
      if (document.visibilityState !== "visible") return;
      const mine = ++seq;
      try {
        const res = await fetch("/api/catalog/enrich", { cache: "no-store" });
        const r = await res.json().catch(() => null);
        if (stopped || mine < latest) return;
        latest = mine;
        if (!res.ok || !r) { setResult(r?.error ?? `Could not read enrich progress (${res.status})`); return; }
        setEnrich(r);
        // Keep polling while the panel is open (a job started from it must be followed too — the
        // interval used to stop for good on the first poll after an earlier job had finished, so
        // Start showed no progress until the panel was reopened); refresh the page once per finish.
        if (r.running) wasRunning = true;
        else if (wasRunning && r.total) { wasRunning = false; router.refresh(); }
      } catch { if (!stopped) setResult("Could not reach the server"); }
    }, 1500);
  ...
  async function addSkus() {
    ...
    let r: { results?: { status: string; sku: string }[]; error?: string };
    try { r = await fetch("/api/catalog/add", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ skus, category }) }).then((x) => x.json()); }
    catch { setBusy(false); setResult("Could not reach the server"); return; }
    setBusy(false);
    if (r.error || !Array.isArray(r.results)) { setResult(r.error ?? "Unexpected reply"); return; }
    const results = r.results;
    const added = results.filter((x: { status: string }) => x.status === "added").length;
    const nf = results.filter((x) => x.status === "not-found").map((x) => x.sku);
    const inv = results.filter((x) => x.status === "invalid").map((x) => x.sku);
    setResult(`${added} added, ${results.length - added - nf.length - inv.length} already present${nf.length ? `, not found under our labelers: ${nf.join(", ")}` : ""}${inv.length ? `, not catalog numbers: ${inv.join(", ")}` : ""}`);
    router.refresh();
  }
  async function importPricing(f: File | null, url?: string) {
    ...
    const r = await fetch("/api/pricing/import", { method: "POST", body: fd }).then((r) => r.json()).catch(() => ({ error: "Could not reach the server" }));
    setBusy(false);
  ...
  async function importSizes(f: File | null, url?: string) {
    ...
    if (f) fd.append("file", f);
    if (url) fd.append("sheetUrl", url);
    const r = await fetch("/api/competitor-sizes/import", { method: "POST", body: fd }).then((r) => r.json()).catch(() => ({ error: "Could not reach the server" }));
    setBusy(false);
  ...
  async function startEnrich() {
    ...
    try {
      const r = await fetch("/api/catalog/enrich", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ onlyMissing: true }) });
      if (!r.ok) { const j = await r.json().catch(() => ({})); setEnrich(null); setResult(j.error ?? `Could not start (${r.status})`); }
    } catch { setEnrich(null); setResult("Could not reach the server"); }
  }
  ...
  return (
    <div className="relative" ref={popover}>
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {canManage && <button type="button" className="btn-secondary" onClick={() => setPanel(panel === "enrich" ? null : "enrich")}>Enrich from GUDID</button>}
        {canImportCost && <button type="button" className="btn-secondary" onClick={() => setPanel(panel === "pricing" ? null : "pricing")}>Pricing</button>}
        {canManage && <button type="button" className="btn-secondary" onClick={() => setPanel(panel === "sizes" ? null : "sizes")}>Competitor sizes</button>}
        {canManage && <button type="button" className="btn-primary" onClick={() => setPanel(panel === "add" ? null : "add")}>Add SKUs</button>}
      </div>
      {panel && (
        <div role="dialog" aria-label={panel === "add" ? "Add SKUs" : panel === "pricing" ? "Pricing import" : panel === "sizes" ? "Competitor sizes import" : "Enrich from GUDID"} className="absolute right-0 top-12 z-20 w-[min(440px,calc(100vw-2rem))] card p-5" style={{ boxShadow: "var(--shadow-lg)" }}>
          {panel === "add" && (
            <>
              ...
              <div className="flex justify-end gap-2 mt-3"><button type="button" className="btn-ghost" onClick={() => setPanel(null)}>Close</button><button type="button" className="btn-primary" disabled={busy || !skus.trim()} onClick={addSkus}>{busy ? "Looking up…" : "Add"}</button></div>
            </>
          )}
          {panel === "pricing" && (
            <>
              ...
              <div className="flex gap-2">
                ...
                <button type="button" className="btn-primary" disabled={busy || !pricingUrl.trim()} onClick={() => importPricing(null, pricingUrl)}>{busy ? "Importing…" : "Import"}</button>
              </div>
              <div className="flex justify-end mt-3"><button type="button" className="btn-ghost" onClick={() => setPanel(null)}>Close</button></div>
            </>
          )}
          {panel === "sizes" && (
            <>
              ...
              <div className="flex gap-2">
                ...
                <button type="button" className="btn-primary" disabled={busy || !sizesUrl.trim()} onClick={() => importSizes(null, sizesUrl)}>{busy ? "Importing…" : "Import"}</button>
              </div>
              <SizeCoverage />
              <div className="flex justify-end mt-3"><button type="button" className="btn-ghost" onClick={() => setPanel(null)}>Close</button></div>
            </>
          ...
          {panel === "enrich" && (
            <>
              ...
              <div className="flex justify-end gap-2 mt-3"><button type="button" className="btn-ghost" onClick={() => setPanel(null)}>Close</button><button type="button" className="btn-primary" disabled={enrich?.running} onClick={startEnrich}>Start</button></div>
            </>
          )}
