...
export function GoogleCard({ status }: { status: { configured: boolean; email: string | null; project: string | null; folderId: string | null; canWrite: boolean } }) {
  ...
  async function runTest() {
    setBusy(true);
    const r = await fetch("/api/google?test=1").then((r) => r.json());
    setTest(r.test);
    setBusy(false);
  }
  return (
    <Card title="Google Sheets & Drive" subtitle="Free for anyone demoing — no Excel licence needed" actions={status.configured ? <button className="btn-secondary" onClick={runTest} disabled={busy}>{busy ? "Testing…" : "Test connection"}</button> : null}>
      ...
      {test && <div className={`mt-3 rounded-lg px-3 py-2 text-[12.5px] ${test.ok ? "bg-exact-soft text-exact" : "bg-none-soft text-none"}`}>{test.message}</div>}
      {!status.canWrite && (
