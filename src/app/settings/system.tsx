...
export function SystemCard() {
  ...
  // 20 s poll while the tab is visible; out-of-order replies are dropped.
  const seq = useRef(0);
  const load = useCallback(async () => {
    const mine = ++seq.current;
    try { const r = await fetch("/api/system", { cache: "no-store" }); const j = await r.json().catch(() => ({})); if (mine !== seq.current) return; if (r.ok) setS(j); else setMsg(j.error ?? `Could not load system status (${r.status})`); }
    catch { if (mine === seq.current) setMsg("Could not reach the server"); }
  }, []);
  useEffect(() => {
    load();
  ...
  async function act(action: string) {
    ...
    try { const r = await fetch("/api/system", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action }) }); const j = await r.json().catch(() => ({})); setMsg(r.ok ? (action === "evaluate-alerts" ? `Evaluated: ${j.firing?.length ?? 0} firing, ${j.resolved ?? 0} resolved` : action === "retry-failed" ? `${j.resumed} job(s) queued again` : "Queued") : j.error ?? `Failed (${r.status})`); load(); }
    catch { setMsg("Could not reach the server"); } finally { setBusy(false); }
  }
  async function runFeed(feed: string) {
    if (busy) return;
    setBusy(true); setMsg(`Queued ${feed}…`);
    try { const r = await fetch("/api/feeds", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ feed, force: true }) }); const j = await r.json().catch(() => ({})); setMsg(r.ok ? (j.alreadyQueued ? `${feed} is already queued` : `${feed} queued`) : j.error ?? `Failed (${r.status})`); setTimeout(load, 3000); }
    catch { setMsg("Could not reach the server"); } finally { setBusy(false); }
  }
  if (!s) return <Card title="System"><div className="text-muted text-[13px]" aria-busy={!msg}>{msg ?? "Loading…"}</div></Card>;
  const stalled = s.jobs.queues.filter((q) => q.oldestReadySeconds !== null && q.oldestReadySeconds > 900);
  return (
    <Card title="System" subtitle="Background jobs, feeds and monitoring. Everything here is also on /api/health (liveness), /api/metrics (Prometheus) and /api/observability/export (NDJSON)." actions={<button type="button" className="btn-ghost !py-1 !text-[12px]" disabled={busy} onClick={() => act("evaluate-alerts")}>Evaluate alerts now</button>}>
      {msg && <div role="status" className="text-[12.5px] text-ink-2 mb-3">{msg}</div>}
      ...
      {s.jobs.failures.length > 0 && (
        <div className="mb-4 text-[12px]">
          <div className="flex items-center justify-between"><span className="text-muted">{s.jobs.failures.length} recent failure(s)</span><button type="button" className="btn-ghost !py-0.5 !text-[11px]" disabled={busy} onClick={() => act("retry-failed")}>Retry failed jobs</button></div>
      ...
      <table className="table text-[12px] mb-4">
        <thead><tr><th>Feed</th><th>Source</th><th>Schedule (UTC)</th><th>Last OK</th><th>Last run</th><th><span className="sr-only">Actions</span></th></tr></thead>
        <tbody>{s.feeds.map((f) => (
          <tr key={f.name}>
            ...
            <td className="text-right">{f.source.kind !== "none" && <button type="button" className="btn-ghost !py-0.5 !text-[11px]" disabled={busy} aria-label={`Run ${f.title} now`} onClick={() => runFeed(f.name)}>Run now</button>}</td>
          </tr>
        ))}</tbody>
      </table>
      ...
      <dl className="grid grid-cols-[150px_1fr] gap-y-1.5 text-[12.5px]">
        ...
        <dt className="text-muted">openFDA</dt><dd>{s.openfda.rpm} req/min budget · cache TTL {s.openfda.cacheTtlDays} days <button type="button" className="btn-ghost !py-0 !text-[11px] ml-2" disabled={busy} onClick={() => act("refresh-gudid")}>Refresh stale records</button></dd>
