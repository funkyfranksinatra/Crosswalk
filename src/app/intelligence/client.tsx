...
export function Intelligence({ sku: initialSku, accountId }: { sku: string; accountId: string }) {
  ...
  return (
    <>
      <PageHeader eyebrow="Competitive intelligence" title="Competitor pricing" description="Append-only, dated observations with provenance. Confidence decays by source half-life; the summary says whether a price is known for this account, a market estimate, or weak." actions={<label className="btn-secondary cursor-pointer">Import .xlsx / .csv<input type="file" accept=".xlsx,.csv" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) importFile(f); }} /></label>} />
