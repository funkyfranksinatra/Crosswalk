...
export function DelegationPanel({ onChange }: { onChange?: () => void }) {
  ...
  const canDelegate = can("approve_discount");
  ...
  const load = useCallback(async () => { try { const r = await fetch("/api/approvals/delegations?all=1", { cache: "no-store" }); if (r.ok) setData(await r.json()); else setErr((await r.json().catch(() => ({}))).error ?? `Could not load delegations (${r.status})`); } catch { setErr("Could not reach the server"); } }, []);
  ...
  async function revoke(id: string) { if (!window.confirm("Revoke this delegation?")) return; try { const r = await fetch(`/api/approvals/delegations/${id}`, { method: "DELETE" }); if (!r.ok) setErr((await r.json().catch(() => ({}))).error ?? `Could not revoke (${r.status})`); } catch { setErr("Could not reach the server"); } load(); onChange?.(); }
  if (!data) return err ? <div role="alert" className="mb-4 text-[12.5px] text-none">{err}</div> : null;
  ...
  return (
    <Card className="mb-4" padded={false}>
      <div className="flex items-center gap-3 px-4 py-2.5 text-[12.5px]">
        ...
        <button type="button" className="btn-ghost !py-0.5 ml-auto text-[12px]" disabled={!canDelegate} title={canDelegate ? undefined : needs("approve_discount")} aria-expanded={open} onClick={() => setOpen(!open)}>{open ? "Close" : "Delegate my approvals"}</button>
      </div>
      {open && (
        <div className="border-t border-line-2 px-4 py-3 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-[1fr_1fr_140px_140px_1fr_auto] gap-2 items-end text-[12.5px]">
          ...
          <button type="button" className="btn-primary" disabled={!form.toUserId || !form.endsAt || busy} onClick={create}>{busy ? "Saving…" : "Save"}</button>
          {err && <div role="alert" className="md:col-span-2 xl:col-span-6 text-none">{err}</div>}
        </div>
      )}
      {(given.length > 0 || received.length > 0) && (
        <div className="border-t border-line-2 px-4 py-2 flex flex-wrap gap-2 text-[12px]">
          {[...given, ...received].map((d) => <span key={d.id} className="inline-flex items-center gap-1.5 rounded-md border border-line px-2 py-1"><Chip tone={d.state === "active" ? "exact" : "alt"}>{d.state}</Chip>{d.from.name} → {d.to.name} · {d.startsAt.slice(0, 10)} – {d.endsAt.slice(0, 10)}{d.reason ? ` · ${d.reason}` : ""}{(d.fromUserId === data.me || data.admin) && <button type="button" className="text-none ml-1" onClick={() => revoke(d.id)}>revoke</button>}</span>)}
        </div>
      )}
    </Card>
  );
