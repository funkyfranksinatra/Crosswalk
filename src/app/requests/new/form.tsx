...
export function NewRequestForm({ pricebooks, llmAvailable }: { pricebooks: { id: string; name: string; entries: number }[]; llmAvailable: boolean }) {
  ...
  const runPreview = useCallback(async (extra?: { file?: File; sheetUrl?: string; csvText?: string }) => {
    ...
    setBusy(false);
    if (!res.ok) { setError({ message: data.error, hint: data.hint }); return; }
    setPreview(data);
    ...
    <div className="grid grid-cols-[1.35fr_1fr] gap-4 items-start">
      <div className="space-y-4">
        <Card title="1 · Competitor usage" subtitle="A product-code column and a quantity column. Header names are detected; repeated codes are merged. Google Sheets is free — no Excel needed.">
          <div className="flex items-center gap-1 mb-4 rounded-lg bg-panel-2 p-1 w-fit">
            {tabs.map(([k, label]) => (
              <button key={k} onClick={() => { setMode(k); setPreview(null); setError(null); }} className={`rounded-md px-3 py-1.5 text-[12.5px] font-medium transition-colors ${mode === k ? "bg-panel shadow-[var(--shadow)] text-ink" : "text-muted hover:text-ink"}`}>{label}</button>
            ))}
          </div>

          {mode === "sheet" && (
            <div>
              <label className="label">Google Sheets link</label>
              <div className="flex gap-2">
                <input className="input mono" placeholder="https://docs.google.com/spreadsheets/d/…/edit#gid=0" value={sheetUrl} onChange={(e) => setSheetUrl(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") runPreview(); }} />
                <button className="btn-secondary" disabled={!sheetUrl.trim() || busy} onClick={() => runPreview()}>{busy ? "Reading…" : "Read sheet"}</button>
              </div>
          ...
          {mode === "file" && (
            ...
            >
              <input type="file" accept=".xlsx,.xlsm,.csv,text/csv" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) onFile(f); }} />
          ...
          {mode === "paste" && (
            <div>
              <label className="label">Paste cells copied from Google Sheets or Excel</label>
              <textarea className="input mono h-36" placeholder={"ProductCode\tQuantity\n1DLMC05\t1\n112660\t10"} value={csvText} onChange={(e) => setCsvText(e.target.value.includes("\t") ? e.target.value.replace(/\t/g, ",") : e.target.value)} />
              <div className="flex justify-end mt-2"><button className="btn-secondary" disabled={!csvText.trim() || busy} onClick={() => runPreview()}>{busy ? "Reading…" : "Read pasted cells"}</button></div>
            </div>
          )}

          {error && (
            <div className="mt-3 rounded-lg bg-none-soft text-none px-3 py-2 text-[13px]">
                ...
                <table className="table">
                  ...
                  <tbody>
                    {preview.lines.map((l, i) => (
                      <tr key={l.cfnNorm}><td className="mono text-muted">{i + 1}</td><td className="mono">{l.rawCode}</td><td className="mono text-muted">{l.cfnNorm !== l.rawCode ? l.cfnNorm : ""}</td><td className="mono text-right">{l.quantity}</td><td className="mono text-right text-muted">{l.estPrice != null ? `$${l.estPrice.toFixed(2)}` : "—"}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input className="input mono" placeholder="0001880967" value={form.accountNumber} onChange={(e) => setForm({ ...form, accountNumber: e.target.value })} />
              </div>
            </div>
            <div>
              <label className="label">Account / Group / IDN name</label>
              <input className="input" placeholder="Memorial Sloan Kettering" value={form.accountName} onChange={(e) => setForm({ ...form, accountName: e.target.value })} />
            </div>
            <div>
              <label className="label">Pricebook</label>
              <select className="input" value={form.pricebookId} onChange={(e) => setForm({ ...form, pricebookId: e.target.value })}>
            ...
            <div>
              ...
              <input className="input" placeholder="Your name" value={form.createdBy} onChange={(e) => setForm({ ...form, createdBy: e.target.value })} />
            </div>
          </div>
        </Card>
        <Card title="3 · Matching">
          <label className={`flex items-start gap-3 ${llmAvailable ? "" : "opacity-60"}`}>
          ...
          <button className="btn-primary w-full justify-center mt-5" disabled={!preview || busy} onClick={submit}>
