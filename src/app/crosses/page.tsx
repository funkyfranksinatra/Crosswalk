...
export default async function CrossesPage({ searchParams }: { searchParams: Promise<{ q?: string; type?: string; company?: string; source?: string }> }) {
  ...
  return (
    <>
      <PageHeader eyebrow="Ground truth" title="Known cross references" description="Human-curated crosses from the Endomechanical review sheets. They take precedence over attribute matching and act as the evaluation set for tuning the matcher." />
      ...
      <Card padded={false}>
        ...
        {rows.length === 0 ? <Empty title="No crosses match" /> : (
          <table className="table">
            <thead><tr><th>Our SKU</th><th>Our description</th><th>Match</th><th>Competitor</th><th>Their code</th><th>Their description</th><th>Notes</th><th>Sheet</th></tr></thead>
            <tbody>
              {rows.map((r) => (
