...
export async function POST(req: Request) {
  return handle("manage_contracts", async (actor) => {
    ...
    const a = await prisma.account.create({ data: { name: requireText(b.name, "name"), accountNumber: optText(b.accountNumber, "accountNumber", 40), type: oneOf(b.type, ACCOUNT_TYPES, "type", "SOLD_TO"), parentAccountId: str(b.parentAccountId), territory: optText(b.territory, "territory", 80), segment: optText(b.segment, "segment", 80), region: optText(b.region, "region", 80), country: requireText(b.country ?? "US", "country", 2), currency: currencyCode(b.currency), isStrategic: Boolean(b.isStrategic), ownerUserId: str(b.ownerUserId) } });
