...
export async function GET(req: Request) {
  ...
  if (format === "csv") {
    ...
    return new Response(toCsv(rows), { headers: { "content-type": "text/csv; charset=utf-8", "content-disposition": `attachment; filename="CRACR_Competitor_Sizes.csv"` } });
  }
  const buffer = await competitorSizesTemplate();
  return new Response(new Uint8Array(buffer), {
    headers: {
      "content-type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      "content-disposition": `attachment; filename="CRACR_Competitor_Sizes.xlsx"`,
    },
  });
}
