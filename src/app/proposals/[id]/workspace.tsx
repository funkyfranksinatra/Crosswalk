...
export function ProposalWorkspace({ id }: { id: string }) {
  ...
  // Reloads are sequenced: a reply from an older reload never replaces a newer one (two tabs,
  // a slow PATCH readback racing a fresh GET).
  const seq = useRef(0);
  const load = useCallback(async () => {
    ...
    try {
      ...
      // A load failure is reported on its own; it never clears an action error (a refused
      // price edit used to be wiped by the reload that followed it, so the user saw nothing).
      if (!r.ok) { setLoadErr(j.error ?? `Failed to load (${r.status})`); setLoadStatus(r.status); return; }
      setP(j); setLoadErr(null); setLoadStatus(200);
      const sr = await fetch(`/api/proposals/${id}/scenarios`, { cache: "no-store" });
    ...
    } catch { if (mine === seq.current) { setLoadErr("Could not reach the server"); setLoadStatus(0); } }
  }, [id]);
  useEffect(() => { load(); }, [load]);
  // Stale tabs: a workspace left open in another tab re-reads the proposal when it regains
  // focus, so a colleague's (or your own other tab's) price change shows before you edit.
  useEffect(() => {
  ...
  if (loadErr && !p) return <Empty title={loadStatus === 404 ? "Proposal not found" : loadStatus === 403 ? "You cannot open this proposal" : "Could not open proposal"}>{loadStatus === 404 ? "It may have been deleted, or it is outside your book of business." : loadErr} <Link className="text-accent" href="/proposals">Back to proposals</Link></Empty>;
  if (!p) return <div className="shimmer h-40 rounded-xl" aria-busy="true" aria-label="Loading proposal" />;
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Actions p={p} busy={busy} editable={editable} call={call} onError={setErr} />}
      />
      {err && <div role="alert" className="mb-4 rounded-lg bg-none-soft text-none px-4 py-2.5 text-[13px] flex items-start gap-3"><span className="flex-1">{err}</span><button type="button" className="text-[12px] underline" onClick={() => setErr(null)}>Dismiss</button></div>}
      {loadErr && p && <div role="alert" className="mb-4 rounded-lg bg-alt-soft text-alt px-4 py-2.5 text-[13px]">Showing the last loaded state — the latest refresh failed: {loadErr}</div>}
      ...
      <div className="sticky top-0 z-10 -mx-2 px-2 pb-3 pt-1 bg-paper/95 backdrop-blur">
        ...
        {scenario && <div className="mt-2 text-[12px] text-info" role="status">Viewing scenario <b>{scenario.scenario.name}</b> — the proposal&apos;s own prices are unchanged. <button type="button" className="underline" onClick={() => setScenario(null)}>Back to proposal</button></div>}
      </div>

      {/* Scenarios + view toggles */}
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        <span className="eyebrow" id="scenarios-label">Scenarios</span>
        <button type="button" aria-pressed={!scenario} className={`chip ${!scenario ? "bg-accent-soft text-accent-ink" : "bg-line-2 text-muted"}`} onClick={() => setScenario(null)}>Proposal</button>
        {scenarios.map((s) => <button key={s.scenario.id} type="button" aria-pressed={scenario?.scenario.id === s.scenario.id} className={`chip ${scenario?.scenario.id === s.scenario.id ? "bg-accent-soft text-accent-ink" : "bg-line-2 text-muted"}`} onClick={() => setScenario(s)}>{s.scenario.name}</button>)}
        {p.permissions.editPricing && (
          <select className="input !w-auto !py-1 !text-[12px]" aria-label="New scenario" disabled={busy} value="" onChange={(e) => { if (e.target.value) call(`/api/proposals/${id}/scenarios`, { method: "POST", body: JSON.stringify({ kind: e.target.value }) }); }}>
        ...
        {scenario && editable && <button type="button" className="btn-secondary !py-1 !text-[12px]" disabled={busy} onClick={() => { if (window.confirm(`Apply "${scenario.scenario.name}" to the proposal? Every included line takes the scenario's price.`)) call(`/api/proposals/${id}/scenarios/${scenario.scenario.id}/apply`, { method: "POST" }).then(() => setScenario(null)); }}>Apply scenario to proposal</button>}
        {scenario && p.permissions.editPricing && <button type="button" className="btn-ghost !py-1 !text-[12px]" disabled={busy} onClick={() => { if (window.confirm(`Delete scenario "${scenario.scenario.name}"?`)) call(`/api/proposals/${id}/scenarios/${scenario.scenario.id}`, { method: "DELETE" }).then(() => setScenario(null)); }}>Delete</button>}
        ...
        <button type="button" className="btn-ghost !py-1 !text-[12px]" aria-expanded={showAudit} onClick={() => setShowAudit((v) => !v)}>{showAudit ? "Hide" : "Show"} audit trail</button>
      </div>
...
/**
 * Download through fetch so a refusal (tax recalculation pending, approval incomplete) is
 * shown in the workspace instead of navigating the tab to a JSON error document.
 */
async function downloadFile(url: string, onError: (m: string) => void) {
  try {
    const r = await fetch(url, { cache: "no-store" });
    if (!r.ok) { const j = await r.json().catch(() => ({})); onError(j.error ?? `Export refused (${r.status})`); return; }
    const blob = await r.blob();
    const name = /filename="?([^";]+)"?/.exec(r.headers.get("content-disposition") ?? "")?.[1] ?? "quote";
...
function Actions({ p, busy, editable, call, onError }: { p: Proposal; busy: boolean; editable: boolean; call: (path: string, init?: RequestInit) => Promise<unknown>; onError: (m: string) => void }) {
  const [outcome, setOutcome] = useState(false);
  const id = p.id;
  const unpriced = p.lines.filter((l) => l.included && l.proposedPrice == null).length;
  return (
    <div className="flex items-center gap-2 flex-wrap justify-end">
      {editable && <button type="button" className="btn-primary" disabled={busy || unpriced > 0} title={unpriced > 0 ? `${unpriced} included line${unpriced === 1 ? " has" : "s have"} no proposed price — price or exclude ${unpriced === 1 ? "it" : "them"} first` : "Route the priced lines to the deal desk"} onClick={() => call(`/api/proposals/${id}/submit`, { method: "POST", body: JSON.stringify({}) })}>Submit for approval</button>}
      {!editable && p.permissions.editPricing && !["WON", "LOST"].includes(p.status) && <button type="button" className="btn-secondary" disabled={busy} title={p.lockedAt ? "Locked while another submission is in progress" : "Reopen for editing — pending approvals are withdrawn"} onClick={() => { if (window.confirm("Reopen this proposal? Pending approvals are withdrawn and it has to be resubmitted.")) call(`/api/proposals/${id}/reopen`, { method: "POST", body: JSON.stringify({ reason: "reopened from workspace" }) }); }}>Reopen</button>}
      {p.permissions.export && (
        <span className="inline-flex rounded-md overflow-hidden" title={p.finalize.ok ? "Download the approved quote" : `Locked: ${p.finalize.reason}`}>
          <button type="button" className="btn-secondary !rounded-r-none" disabled={!p.finalize.ok} onClick={() => downloadFile(`/api/proposals/${id}/export?format=pdf`, onError)}>Quote PDF</button>
          <button type="button" className="btn-secondary !rounded-l-none border-l-0" disabled={!p.finalize.ok} onClick={() => downloadFile(`/api/proposals/${id}/export?format=xlsx`, onError)}>.xlsx</button>
        </span>
      )}
      {p.permissions.export && p.finalize.ok && <button type="button" className="btn-secondary" disabled={busy} onClick={() => call(`/api/proposals/${id}/push-crm`, { method: "POST" })} title={p.integrations.crm.note}>Push to CRM</button>}
      {p.permissions.outcomes && p.finalize.ok && !p.outcome && <button type="button" className="btn-secondary" aria-haspopup="dialog" onClick={() => setOutcome(true)}>Record outcome</button>}
      {p.permissions.editPricing && ["WON", "LOST", "APPROVED", "REJECTED"].includes(p.status) && <button type="button" className="btn-ghost" disabled={busy} onClick={() => call(`/api/proposals/${id}/version`, { method: "POST" }).then((r) => { const v = r as { id?: string } | null; if (v?.id) window.location.href = `/proposals/${v.id}`; })}>New version</button>}
...
function OutcomeDialog({ p, onClose, call }: { p: Proposal; onClose: () => void; call: (path: string, init?: RequestInit) => Promise<unknown> }) {
  ...
  useEffect(() => {
    ...
    const k = (e: KeyboardEvent) => {
      ...
      const f = [...box.current.querySelectorAll<HTMLElement>("select, input, button")].filter((el) => !el.hasAttribute("disabled"));
      if (!f.length) return;
  ...
  async function save() {
    if (saving) return;
    setSaving(true);
    try { const r = await call(`/api/proposals/${p.id}/outcome`, { method: "POST", body: JSON.stringify(form) }); if (r) onClose(); } finally { setSaving(false); }
  }
  return (
    <div className="fixed inset-0 z-30 bg-ink/30 flex items-center justify-center p-3" onClick={onClose}>
      <div ref={box} role="dialog" aria-modal="true" aria-labelledby="outcome-title" className="card p-5 w-full max-w-[460px]" onClick={(e) => e.stopPropagation()}>
        <div className="font-semibold mb-1" id="outcome-title">Record outcome for {p.reference}</div>
        ...
        <select className="input mb-2" aria-label="Outcome" value={form.outcome} onChange={(e) => setForm({ ...form, outcome: e.target.value })}><option value="WON">Won</option><option value="LOST">Lost</option><option value="NO_DECISION">No decision</option></select>
        {form.outcome === "LOST" && <input className="input mb-2" aria-label="Winning competitor" placeholder="Winning competitor" value={form.competitorName} onChange={(e) => setForm({ ...form, competitorName: e.target.value })} />}
        <input className="input mb-2" aria-label="Price reason" placeholder="Price reason (e.g. 8% above competitor on mesh)" value={form.priceReason} onChange={(e) => setForm({ ...form, priceReason: e.target.value })} />
        <input className="input mb-2" aria-label="Commercial reason" placeholder="Commercial reason (e.g. incumbent bundle, clinical preference)" value={form.commercialReason} onChange={(e) => setForm({ ...form, commercialReason: e.target.value })} />
        {form.outcome === "WON" && <label className="label">Contract term (months)<input className="input" type="number" min={1} max={120} value={form.contractMonths} onChange={(e) => setForm({ ...form, contractMonths: Number(e.target.value) })} /></label>}
        <div className="flex justify-end gap-2 mt-3"><button type="button" className="btn-ghost" onClick={onClose}>Cancel</button><button type="button" className="btn-primary" disabled={saving} onClick={save}>{saving ? "Saving…" : "Save"}</button></div>
      </div>
    </div>
  );
}
...
function LineRow({ l, p, proposed, margin, auth, belowFloor, editable, isOpen, onOpen, onPrice, onInclude, onRecommend, onNote, approvals }: { l: Line; p: Proposal; proposed: string | number | null; margin: string | number | null; auth: string | null; belowFloor: boolean; editable: boolean; isOpen: boolean; onOpen: () => void; onPrice: (v: string | null) => void; onInclude: (v: boolean) => void; onRecommend: (strategy: string, adj: number | null, just: string | null) => void; onNote: (v: string | null) => void; approvals: Approval[] }) {
  ...
  // Reset the draft on every reload (the line object changes identity), so a value the server
  // refused ("-5") does not linger in the box next to the price that is really stored.
  useEffect(() => { setDraft(proposed == null ? "" : String(proposed)); }, [proposed, l]);
  ...
  return (
    <>
      <tr className={`${!l.included ? "bg-panel-2 [&>td]:text-muted" : ""} ${isOpen ? "bg-accent-soft/30" : ""}`}>
        <td className="cursor-pointer" onClick={onOpen}><button type="button" className="mono font-medium text-left" aria-expanded={isOpen} onClick={(e) => { e.stopPropagation(); onOpen(); }}>{l.competitorCode}</button><div className="text-[11.5px] text-muted truncate max-w-[260px]">{l.competitorName ? `${l.competitorName} · ` : ""}{l.competitorDescription}</div></td>
        ...
        <td className="text-right">
          {editable ? <input className={`input mono !w-28 text-right !py-1 ${belowFloor ? "!border-none-soft !bg-none-soft/40" : ""}`} inputMode="decimal" aria-label={`Proposed price for ${l.competitorCode}`} aria-invalid={belowFloor || undefined} title={belowFloor ? "Below the floor — needs approval" : undefined} value={draft} onChange={(e) => setDraft(e.target.value)} onBlur={commit} onKeyDown={(e) => { if (e.key === "Enter") (e.target as HTMLInputElement).blur(); }} /> : <span className={`mono ${belowFloor ? "text-none" : ""}`}>{fmtMoney(proposed, p.currency)}</span>}
  ...
  return (
    <div className="bg-panel border-t border-line-2 px-5 py-4 text-[13px]">
      <div className="flex items-center gap-1 mb-3 flex-wrap">
        {tabs.map(([k, t]) => <button key={k} type="button" aria-pressed={tab === k} className={`chip ${tab === k ? "bg-accent-soft text-accent-ink" : "bg-line-2 text-muted"}`} onClick={() => setTab(k)}>{t}</button>)}
        <span className="ml-auto flex items-center gap-2">
          ...
          {editable && <button type="button" className="btn-ghost !py-1 !text-[12px]" onClick={() => onInclude(!l.included)}>{l.included ? "Exclude line" : "Include line"}</button>}
        </span>
      </div>
      {tab === "rec" && (
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
          <div>
            ...
            {rec && <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-3 text-[12px]">
          ...
          {editable && (
            <div className="card p-3 bg-paper">
              ...
              <select className="input mb-2" aria-label="Strategy" value={strategy} onChange={(e) => setStrategy(e.target.value)}>
              ...
              <button type="button" className="btn-primary w-full justify-center mt-2" onClick={() => onRecommend(strategy, Number(adj) / 100 || null, just || null)}>Apply to this line</button>
            </div>
          )}
        </div>
      )}
      ...
      {tab === "cost" && (cost ? <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-[12px]"><Fact k="Cost" v={fmtMoney(l.cost, p.currency)} /><Fact k="Basis" v={label(cost.kind)} /><Fact k="Specificity" v={cost.specificity ?? "—"} /><Fact k="Note" v={cost.note} /><Fact k="Margin / unit" v={fmtMoney(l.marginAmount, p.currency)} /><Fact k="Margin %" v={fmtPct(l.marginPct)} /><Fact k="Floor" v={fmtMoney(l.floorPrice, p.currency)} /><Fact k="Target" v={fmtMoney(l.targetPrice, p.currency)} /></div> : <span className="text-muted">No cost on file — margin unknown.</span>)}
      {tab === "cross" && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-[12px]">
          ...
          <div className="col-span-2 md:col-span-4 text-[11.5px] text-muted">Only entries in the published crosswalk may be represented to a customer as an equivalence. {l.equivalenceLevel === "NONE" && l.sku ? "This pairing was produced by the cross-reference engine but is not yet approved: ask product marketing to review it under Crosswalk." : ""}</div>
        </div>
      )}
...
function AuditTrail({ id }: { id: string }) {
  ...
  const [err, setErr] = useState<string | null>(null);
  useEffect(() => { let stop = false; fetch(`/api/proposals/${id}/audit`, { cache: "no-store" }).then(async (r) => { const j = await r.json().catch(() => null); if (stop) return; if (!r.ok) setErr((j && j.error) || `Could not load the audit trail (${r.status})`); else setRows(Array.isArray(j) ? j : []); }).catch(() => { if (!stop) setErr("Could not reach the server"); }); return () => { stop = true; }; }, [id]);
  return (
    <Card title="Audit trail" subtitle="Every price change, submission, decision and export — with the recommendation, floor and margin at the time" className="mt-4" padded={false}>
      {err && <div role="alert" className="p-4 text-[12.5px] text-none">{err}</div>}
