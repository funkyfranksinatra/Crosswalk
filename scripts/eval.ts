...
import { parseBin, binSimilarity, heuristicBin, type Bin } from "../src/lib/match/bin";
...
async function main() {
  ...
  const ownBins = own.map((p) => ({ p, bin: parseBin(p.binJson) ?? heuristicBin({ description: p.description, category: p.category }) }));
  ...
  for (const [i, item] of pool.entries()) {
    ...
    if (cp && cp.resolution !== "not-found" && !bin) {
      ...
      const b = await binProduct({ subject: item.code, brand: cp.brand, description: cp.description, manufacturer: cp.manufacturer, gmdnName: cp.gmdnName, sizes: s?.sizes, singleUse: s?.singleUse, sterile: s?.sterile, implantable: s?.implantable, useLlm });
      bin = b.bin;
    ...
    const compBin = mergeBins(bin, heuristicBin({ description: compDesc }));
