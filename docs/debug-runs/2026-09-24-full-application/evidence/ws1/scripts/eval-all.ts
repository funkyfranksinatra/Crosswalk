...
import { parseBin, binSimilarity, heuristicBin, withAccessProfile, type Bin } from "@/lib/match/bin";
import { mergeProfiles, emptyProfile } from "@/lib/match/access";
...
async function main() {
  ...
  for (const [i, item] of pool.entries()) {
    ...
    if (hit3) top3++;
    misses.push(`${hit1 ? "T1" : hit3 ? "T3" : "--"} ` + `${item.code.padEnd(14)} ${item.name.padEnd(18)} expected ${[...item.expected].join("/").padEnd(14)} got ${top.join(", ") || "—"}  | ${(compDesc || "").slice(0, 60)}`);
